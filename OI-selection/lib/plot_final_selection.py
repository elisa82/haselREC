...
#	plt.close()
	...
	for i in np.arange(nGM):
		plt.loglog(TgtPer,np.exp(sampleSmall[i,:]),'g')
	plt.plot(T_CS,np.exp(meanReq),'r',label='CMS')
	plt.plot(T_CS,np.exp(meanReq+2*stdevs),'--r',label=r'CMS $\pm 2\sigma$')
	plt.plot(T_CS,np.exp(meanReq-2*stdevs),'--r')
	plt.plot(TgtPer,meanrecorded,'k',label='Selected')
	plt.plot(TgtPer,meanrecorded_p2sigma,'--k',label=r'Selected $\pm 2\sigma$')
	plt.plot(TgtPer,meanrecorded_n2sigma,'--k')
	plt.xlabel('Period [s]')
	...
	plt.figure()
	plt.rcParams.update({'font.size': 8})
	plt.plot(T_CS,stdevs,'r',label='CMS')
	plt.plot(TgtPer,meanrecorded_eps,'k',label='Selected')
	plt.xlabel('Period [s]')
